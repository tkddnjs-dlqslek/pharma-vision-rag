...
**생성 단계 결과 (2026-09-21, 120질의, 정답 1점 부분 0.5점)**. 고정 3방식은 상위 3페이지만 보고 blind 생성(331건, 중복 제거),
agentic은 BM25 검색과 페이지 열기로 직접 탐색(질문당 평균 도구 2.7회). 아래는 **네 방식 답변 451건을 익명 id로 섞어 서브에이전트
(Sonnet 5) 12명이 blind로 다시 채점한 값**(`scripts/22_blind_rejudge.py`, 기준은 `eval/results/judge_tasks/INSTRUCTIONS.md`와 동일).

| 방식 | 점수 | A 차트 | B 표 | C 본문 | D 멀티홉 | 영 / 한 | 명시 / 모호 기간 | 키 문자열 일치(채점자 무관) |
|---|---|---|---|---|---|---|---|---|
| text_rerank | 0.59 | 0.62 | 0.49 | 0.90 | 0.40 | 0.65 / 0.53 | 0.59 / 0.55 | 0.31 |
| vision | 0.79 | 0.86 | 0.85 | 0.82 | 0.47 | 0.81 / 0.77 | 0.83 / 0.55 | 0.53 |
| hybrid_rerank | 0.75 | 0.79 | 0.81 | 0.90 | 0.40 | 0.77 / 0.73 | 0.80 / 0.50 | 0.47 |
| **agentic (E4)** | **0.87** | 0.86 | **0.93** | 0.72 | **0.90** | 0.86 / 0.88 | 0.91 / 0.68 | 0.53 |

부호 검정(질의와 언어 단위 짝비교): vision 대 text_rerank 42승 17패(p=0.002), 차트만 15승 3패(p=0.008).
agentic 대 vision 26승 12패(p=0.03), 멀티홉만 12승 0패(p=0.0005), 차트만 5승 5패. agentic 대 hybrid_rerank 35승 11패(p=0.0005).
vision 대 hybrid_rerank 19승 14패(p=0.49, 차이 없음).

읽는 법:
- **검색 단계의 비전 우위가 답변 단계까지 남음**: vision이 text_rerank보다 전체와 차트 모두 유의하게 높음.
- **agentic이 전체 1위이고, 우위의 대부분은 멀티홉**: D 0.90 대 0.40~0.47, 고정 방식 누구에게도 한 번도 지지 않음(0승 12~16패).
  단일 검색이 못 푸는 "쌍둥이 페이지"와 "회사 쏠림"을 질의 분해와 문서 필터로 해결함. 키 문자열 일치로도 0.50 대 0.05~0.15.
- **차트는 agentic과 vision이 동률**(0.86, 5승 5패). agentic에는 비전 검색 도구가 없어서, BM25로 후보 페이지를 찾은 뒤 열어서 읽음.
- **본문은 agentic이 가장 낮음**(0.72): BM25가 C04(영국-미국 협정)를 한영 모두 못 찾음. 고정 방식은 dense와 비전 검색으로 찾음.
  agentic에 dense나 비전 검색 도구를 주면 이 약점은 사라질 것으로 보임(미실험).
- **hybrid_rerank는 vision을 넘지 못함**: 검색 단계에서도 R@1이 0.54로 vision 0.62보다 낮았음. 상위 3장만 쓰는 생성 단계에서 그 차이가 드러남.
- **한국어 손실은 text 경로에서만 큼**(-0.12). 비전은 -0.04, agentic은 한국어가 0.02 높음(차이는 작고 원인은 확인 안 함).

**채점 신뢰도**: 1차 채점(메인 세션이 직접 판정, agentic은 arm을 알고 채점)과 blind 재채점의 판정 일치율은 0.88~0.94.
1차가 셀당 더 후하게 준 정도는 text_rerank +0.013, vision +0.004, hybrid_rerank +0.046, **agentic +0.008**.
agentic을 알고 채점한 편향은 거의 없었고, 1차에서 가장 후했던 건 hybrid_rerank였음(0.80에서 0.75로 하락).
재채점 묶음은 B08 기준 정답 수정(두 발행일 병기) 전에 만들어져, "6월 17일"을 언급한 답이 일부 부분 점수를 받았음(영향은 몇 셀).
한계: 생성과 채점 모두 1회(계획서의 3회 반복은 못 함). 채점자끼리 모호 기간 문항의 기준이 조금씩 달랐음.

**v2 검색 벤치마크 (2026-09-20, 1,709p, 60문항 × 한/영 = 120건, `eval/runner.py`)**. 검색 단계만. 생성과 채점은 API 키 대기.

| 변형 | R@1 | R@5 | NDCG@5 | A 차트 R@5 | B 표 R@5 | C 본문 R@5 | D 멀티홉 R@5 | 한 / 영 R@5 |
|---|---|---|---|---|---|---|---|---|
| bm25 (키워드만) | 0.25 | 0.49 | 0.39 | 0.50 | 0.50 | 0.70 | 0.22 | 0.35 / 0.63 |
| text (BGE-M3 dense, 정확 코사인) | 0.35 | 0.67 | 0.53 | 0.70 | 0.72 | 0.90 | 0.28 | 0.63 / 0.71 |
| text_bm25 (dense + bm25, RRF) | 0.35 | 0.69 | 0.54 | 0.75 | 0.70 | 0.90 | 0.32 | 0.59 / 0.78 |
| text_rerank (+ bge-reranker) | 0.35 | 0.70 | 0.54 | 0.72 | 0.78 | 0.90 | 0.30 | 0.65 / 0.75 |
| **vision (Nemotron, 정확 MaxSim)** | **0.62** | 0.84 | **0.75** | **0.97** | 0.90 | 0.90 | 0.42 | 0.82 / 0.87 |
| hybrid (dense + vision, RRF) | 0.44 | 0.84 | 0.67 | 0.90 | 0.93 | 1.00 | 0.38 | 0.81 / 0.87 |
| hybrid_bm25 (text_bm25 + vision) | 0.49 | 0.83 | 0.69 | 0.90 | 0.88 | 1.00 | 0.43 | 0.81 / 0.86 |
| hybrid_rerank (리랭커 + vision, RRF) | 0.54 | **0.86** | 0.73 | 0.93 | **0.95** | 1.00 | 0.40 | 0.82 / 0.90 |

수치는 실패 분석(`docs/FAILURE_ANALYSIS.md`)에서 찾은 gold 누락 5문항(A02, A03, A05, A19, C05)과, 2026-09-21 렌더로 확인한 A01 보도자료 페이지
(`sanofi_2024Q1_pr.pdf` p5, 636으로 표기. 덱 차트는 재작성 값 637)를 반영한 뒤의 값. A01 반영으로 text 계열 차트 R@5가 0.02~0.05 올랐고 결론은 그대로. 반영 전에는 text 계열의
차트 R@5가 0.57~0.70이었고, 누락된 페이지를 주로 텍스트 경로가 찾고 있었기 때문에 **보완이 텍스트 쪽에 유리하게 작용**했다. 그래도 격차는 남음.

질의 단위 짝비교(R@5, 부호 검정): vision 대 text_rerank 전체 28승 10패(p=0.005), 차트만 11승 1패(p=0.006), 표만 7승 2패(p=0.18, 유의하지 않음).
vision 대 hybrid_rerank 6승 7패(차이 없음). text_bm25 대 text 12승 10패(p=0.83, 차이 없음).

읽는 법:
- **핵심 가설은 검색 단계에서 지지됨**: 텍스트 경로를 청킹 개선, 리랭커, 키워드 검색으로 강화해도 차트 문항에서 비전과의 격차가 남는다(0.72 대 0.97).
- **BM25는 도움이 되지 않았음**(시도했고 효과 없음으로 기록). 단독으로는 가장 약하고, dense와 섞어도 유의한 개선이 없으며 리랭커보다 못함.
  원인: 분기와 회사만 다른 쌍둥이 페이지가 어휘를 공유하고, 한국어 질의는 영문 코퍼스와 겹치는 토큰이 제품명과 숫자뿐(bm25 한국어 R@5 0.35).
  섞으면 한영 격차가 오히려 벌어짐(text 6%p에서 text_bm25 20%p).
- **표와 본문은 격차가 작거나 없음**. 표는 방향은 비전 우위지만 이 표본 크기에서는 유의하지 않음.
- **고정 가중치 RRF는 비전 단독의 R@1을 넘지 못함**(0.54 대 0.62). R@5와 표, 본문은 hybrid_rerank가 가장 높음.
- **멀티홉은 모든 변형이 0.22~0.43**. 단일 검색의 구조적 한계. 실패 분석에 따르면 원인이 둘로 갈림: 한 회사의 여러 분기를 묻는 문항은
  전년도 쌍둥이 페이지가 자리를 차지하고, 여러 회사를 비교하는 문항은 상위 5장이 한 회사로 쏠림. 둘 다 E4(agentic)의 문서 필터와 질의 분해가 겨냥하는 문제.
- 리랭커는 표 문항에서 순손실(14건 잃고 8건 회복): 표 구조는 잘 맞추지만 연도와 분기 토큰을 dense보다 약하게 반영함.
- 한국어 질의는 모든 변형에서 영어보다 낮음. 격차는 vision이 가장 작음(5%p).
