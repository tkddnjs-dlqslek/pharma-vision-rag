...
agentic은 BM25 검색과 페이지 열기로 직접 탐색(질문당 평균 도구 2.7회). 채점 기준은 `eval/results/judge_tasks/INSTRUCTIONS.md`.

| 방식 | 판정 점수 | A 차트 | B 표 | C 본문 | D 멀티홉 | 영 / 한 | 키 문자열 일치(채점자 무관) |
|---|---|---|---|---|---|---|---|
| text_rerank | 0.60 | 0.65 | 0.49 | 0.90 | 0.42 | 0.66 / 0.54 | 0.31 |
| vision | 0.79 | 0.86 | 0.85 | 0.80 | 0.53 | 0.81 / 0.78 | 0.53 |
| hybrid_rerank | 0.80 | 0.81 | 0.85 | 0.95 | 0.50 | 0.81 / 0.78 | 0.47 |
| **agentic (E4)** | **0.88** | 0.86 | 0.90 | 0.75 | **0.97** | 0.85 / 0.90 | 0.53 |

읽는 법:
- **검색 단계의 비전 우위가 답변 단계까지 남음**: vision과 hybrid_rerank가 text_rerank보다 유의하게 높음(부호 검정 p<0.01, 차트만 p≈0.02).
- **agentic의 우위는 멀티홉에 집중됨**: D 0.97 대 0.50~0.53, hybrid 대비 16승 0패. 키 문자열 일치로 봐도 0.50 대 0.05~0.15라 채점자와 무관하게 성립.
  단일 검색 파이프라인이 못 푸는 "쌍둥이 페이지"와 "회사 쏠림"을 질의 분해와 문서 필터로 해결함.
- **차트는 agentic이 비전을 넘지 못함**: 판정 점수는 같고(0.86, 4승 5패), 키 일치는 비전이 높음(0.58 대 0.47). agentic에는 비전 검색 도구가 없어 BM25로 페이지를 찾은 뒤 열어봐야 함.
- **본문은 agentic이 가장 낮음**(0.75): BM25가 C04(영국-미국 협정)를 한영 모두 못 찾음. 고정 방식은 dense와 비전 검색으로 찾음.
- **주의: agentic 채점은 blind가 아님.** 고정 3방식은 채점 때 방식을 몰랐지만, agentic 120건은 따로 채점해서 채점자가 알았음.
  같은 기준과 판례(636 대 637은 부분, 한 시점만 답하면 부분 등)를 적용했으나, 전체 0.88 대 0.79의 격차 일부는 여기서 올 수 있음.
  키 문자열 일치는 agentic과 vision이 0.53으로 같음. **확실한 결론은 "멀티홉 우위"까지이고, 전체 우위는 blind 재채점으로 확인할 것.**
- 채점도 서브에이전트로 하려 했으나 사용량 한도로 실패해 메인 세션이 직접 판정함.
...
| text (BGE-M3 dense, 정확 코사인) | 0.35 | 0.66 | 0.52 | 0.68 | 0.72 | 0.90 | 0.28 | 0.63 / 0.69 |
| text_bm25 (dense + bm25, RRF) | 0.35 | 0.67 | 0.53 | 0.70 | 0.70 | 0.90 | 0.32 | 0.57 / 0.77 |
...
| hybrid (dense + vision, RRF) | 0.43 | 0.83 | 0.66 | 0.88 | 0.93 | 1.00 | 0.38 | 0.81 / 0.85 |
| hybrid_bm25 (text_bm25 + vision) | 0.47 | 0.83 | 0.68 | 0.90 | 0.88 | 1.00 | 0.43 | 0.81 / 0.86 |
| hybrid_rerank (리랭커 + vision, RRF) | 0.54 | **0.86** | 0.73 | 0.93 | **0.95** | 1.00 | 0.40 | 0.82 / 0.90 |

수치는 실패 분석(`docs/FAILURE_ANALYSIS.md`)에서 찾은 gold 누락 5문항(A02, A03, A05, A19, C05)을 반영한 뒤의 값. 반영 전에는 text 계열의
