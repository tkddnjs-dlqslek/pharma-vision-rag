...
| agentic (E4, BM25만) | 0.87 | 0.86 | 0.93 | 0.72 | 0.90 | 0.86 / 0.88 | 0.91 / 0.68 | 0.53 |
| **agentic_vision (E4b, BM25 + 비전)** | **0.93** | **0.90** | **0.94** | **0.95** | **0.93** | 0.93 / 0.92 | 0.95 / 0.80 | - |

E4b는 E4와 같은 에이전트에 `search_pages --source vision`(Nemotron 페이지 순위, 원래 질문 문구로만 조회 가능)을 더한 것.
E4b는 나중에 만들어져 2차 채점(r2)에서 채점됨: 새 답변 120건에 1차에서 이미 채점한 답변 40건을 기준점으로 섞음.
기준점 판정 일치 36/40, **r2가 답변당 +0.025 후했음**. 그래서 E4b 0.93은 r1 기준으로 약 0.90으로 읽는 게 안전함.
...
agentic_vision 대 vision 30승 9패(p=0.001), 차트만 5승 5패. agentic_vision 대 agentic 16승 6패(p=0.053), **본문만 8승 1패(p=0.04)**.
...
- **본문은 E4가 가장 낮았고(0.72), 비전 도구를 더한 E4b에서 해소됨(0.95)**: E4는 BM25가 C04(영국-미국 협정)를 한영 모두 못 찾았음.
  E4b는 BM25로 세 번 실패한 뒤 비전 검색으로 20-F p52를 찾음. 본문 문항에서 E4b가 E4에 8승 1패.
- **차트는 비전 도구가 있어도 vision 단독과 동률**(0.90 대 0.86, 5승 5패). vision 단독의 차트 R@5가 이미 0.97이라 반복 탐색으로 더 얻을 여지가 작음.
- E4b 에이전트가 비전 도구를 항상 쓰지는 않았음. 한 묶음(A01~A10 한국어)은 BM25로 충분하다고 판단해 한 번도 안 썼고,
  그중 A03(Roche 2024년 4분기 성장률)은 차트에만 있는 값이라 "코퍼스에 없음"으로 답함. 도구 선택 자체가 에이전트의 약점이 될 수 있음.
...
도구 예산(질문당 10회) 초과는 E4 1셀(A16_en, 11회), E4b 1셀(D08_ko, 14회). API 루프는 10회에서 답을 강제하지만 서브에이전트는 지시만 받음.
초과 셀을 오답으로 치면 E4 0.86, E4b 0.92.
