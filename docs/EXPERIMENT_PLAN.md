...
|---|---|---|---|---|---|---|---|---|
| text (BGE-M3 dense, 정확 코사인) | 0.32 | 0.63 | 0.49 | 0.57 | 0.72 | 0.90 | 0.28 | 0.60 / 0.66 |
| text_rerank (+ bge-reranker) | 0.31 | 0.69 | 0.51 | 0.70 | 0.78 | 0.90 | 0.30 | 0.65 / 0.74 |
| **vision (Nemotron, 정확 MaxSim)** | **0.56** | **0.84** | **0.72** | **0.97** | 0.90 | 0.90 | 0.42 | 0.82 / 0.87 |
| hybrid (dense + vision, RRF) | 0.40 | 0.81 | 0.62 | 0.80 | 0.93 | 1.00 | 0.38 | 0.78 / 0.83 |
| hybrid_rerank (리랭커 + vision, RRF) | 0.51 | 0.84 | 0.70 | 0.88 | 0.95 | 1.00 | 0.40 | 0.79 / 0.90 |

질의 단위 짝비교(R@5, 부호 검정): vision 대 text_rerank 전체 29승 10패 81무(p=0.003), 차트만 12승 1패(p=0.003),
표만 7승 2패(p=0.18, 유의하지 않음). hybrid_rerank 대 text_rerank 23승 4패(p<0.001). vision 대 hybrid_rerank 8승 7패(차이 없음).

읽는 법:
- **핵심 가설은 검색 단계에서 지지됨**: 텍스트 경로를 청킹 개선과 리랭커로 강화해도 차트 문항에서 비전과의 격차가 남는다(0.70 대 0.97).
  text_rerank가 상위 5장에 못 넣은 차트 문항은 8개, vision은 1건(A13 한국어)뿐.
- **표와 본문은 격차가 작거나 없음**. 표는 방향은 비전 우위지만 이 표본 크기에서는 유의하지 않음.
- **고정 가중치 RRF는 비전 단독을 넘지 못함**. R@1은 오히려 낮아짐(0.51 대 0.56). 다만 표와 본문 R@5는 hybrid가 가장 높음.
- **멀티홉은 모든 변형이 0.28~0.42**. 단일 검색의 구조적 한계이며 E4(agentic)의 근거.
- 한국어 질의는 모든 변형에서 영어보다 낮음. 격차는 text_rerank 9%p, vision 5%p.
- v1(172p)의 text_rerank 차트 R@5 0.40은 v2에서 0.70으로 다르게 나왔다. 코퍼스와 문항이 모두 바뀌었기 때문에 직접 비교할 수 없음.
