...
| 형식 | 크기 | R@1 | R@5 | 차트 R@5 | GPU 질의 시간 |
|---|---|---|---|---|---|
| full_fp16 | 18.9GB | 0.641 | 0.878 | 0.975 | 1148ms |
| int8 (자동 선택) | 9.5GB | 0.633 | 0.875 | 0.975 | 118ms |
| pooled | 4.7GB | 0.617 | 0.867 | 0.975 | 9ms |
| pooled_int8 | 2.4GB | 0.617 | 0.867 | 0.975 | 28ms |

- 차트 R@5는 네 형식 모두 0.975로 같음. 압축 손실은 멀티홉과 본문 쪽에서 나옴.
- 다운로드는 int8(9.5GB). 개발 PC(RAM 16GB)에서는 질의마다 9.5GB를 훑어 15분에 120질의를 못 끝냄.
  `scripts/26_pool_local_index.py`로 **int8에서 로컬 병합해 pooled_int8(2.4GB)을 만듦**(12분, GPU 불필요).
...
답변이 기준 정답과 맞았는지와 별개로, 답변의 주장이 그 답변이 본 페이지로 뒷받침되는지를 봄.
표본은 20문항 × 5방식(1회차) = 95개 답변(고정 방식은 같은 상위 3페이지를 공유해 중복 제거). 서브에이전트가 방식과 기준 정답을 모르는 채로,
답변을 주장 단위로 쪼개 supported, unsupported, contradicted로 판정. 고정 방식의 근거는 생성 때 받은 3페이지, 에이전트는 자기가 인용한 페이지.

| 방식 | 답변 | 주장 | 주장 지지율 | 전부 지지 | 모순 포함 |
|---|---|---|---|---|---|
| text_rerank | 20 | 64 | 1.00 | 1.00 | 0.00 |
| vision | 20 | 64 | 1.00 | 1.00 | 0.00 |
| hybrid_rerank | 20 | 70 | 1.00 | 1.00 | 0.00 |
| agentic (E4) | 19 | 71 | 1.00 | 1.00 | 0.00 |
| agentic_vision (E4b) | 20 | 69 | 0.97 | 0.95 | 0.00 |

- **환각은 사실상 없음.** 95개 답변 중 페이지와 모순되는 주장은 0건, 근거 없는 주장은 한 답변에만 있었음.
  방식 간 차이도 없음(불일치 셀 1개). 페이지 이미지를 직접 보고 답하는 구조라 예상대로지만, 확인된 사실로 적어 둠.
- **정답 여부와 근거성은 다른 축**: 표본에서 오답 13건은 전부 "근거는 맞는데 답이 틀린" 경우였음.
  예: A02(2024년 2분기 환율 영향)에서 에이전트는 2024 Q2 보도자료 p27의 -238을 그대로 인용했는데, 기준 정답은 Opella 제외로 재작성된 -201.
  즉 남은 실패는 환각이 아니라 **검색이 데려온 페이지와 기준 정답의 시점 선택** 문제. 메인 세션이 이 2건을 렌더해 직접 확인함.
- 한계: 고정 방식은 받은 3페이지 전부를 근거로 채점하고 에이전트는 인용한 페이지만 보므로, 에이전트 쪽이 조금 유리함.
  표본 95건으로 천장에 붙어 방식 간 비교는 의미가 없음. 채점자 간 주장 쪼개기 편차는 측정하지 않음.

**인용 정확도 (채점 불필요, 전 방식 전 회차 120셀)**

| 방식 | 인용 페이지 중 gold | gold 1개 이상 인용 | 인용 없음 |
|---|---|---|---|
| text_rerank | 0.73~0.74 | 0.63~0.64 | 27~28건 |
| vision | 0.77~0.79 | 0.87~0.88 | 6~7건 |
| hybrid_rerank | 0.77 | 0.85 | 8~9건 |
| agentic (E4) | 0.86~0.91 | 0.88~0.94 | 2~4건 |
| agentic_vision (E4b) | 0.91~0.93 | 0.94~0.95 | 0~3건 |

- 범위는 회차 간 값. 텍스트 경로는 120건 중 27건에서 아예 인용을 못 함(근거 페이지를 못 찾음).
- 에이전트가 가장 높음. 페이지를 열어 확인한 뒤 답하는 절차가 인용 품질로 이어짐.
- RAGAS 대응: context recall과 precision은 검색 지표(R@k, 아래 표), faithfulness는 위 표, answer correctness는 생성 단계 표에 해당.
  RAGAS 라이브러리 자체는 API 키가 없어 돌리지 않았고, 같은 개념을 서브에이전트 채점과 gold로 계산함.
